...
//	$teamImage = $_FILES["imagedata"];
   ...
   // $nimage=$_FILES['imagedata']['name'];
	...
	$sqlnew = "update documents set title='$request->title' , author='$request->author' , description='$request->description' , keywords='$request->keywords' , type='$request->type' where id='$request->id'";
	$result=mysqli_query($con,$sqlnew);
//	if (!($stmt = $db->conn->prepare($sqlnew))) {
//	    throw new Exception("Prepare failed: (" . $db->conn->errno . ") ");
//	}
//	if (!$stmt->execute()) {
//	    throw new Exception("Execute failed: (" . $stmt->errno . ") ");
